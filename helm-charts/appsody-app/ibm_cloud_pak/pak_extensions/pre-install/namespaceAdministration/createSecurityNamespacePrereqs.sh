...
if supports_scc; then
  ...
  if command -v oc >/dev/null 2>&1 ; then
    oc adm policy add-scc-to-group kitura-scc system:serviceaccounts:$namespace
  else
...
if supports_psp; then
  ...
  sed 's/{{ NAMESPACE }}/'$namespace'/g' kitura-rb.yaml > $namespace-kitura.yaml
  ...
  kubectl create -f $namespace-kitura.yaml -n $namespace

  # Clean up - delete the temporary yaml file.
  rm $namespace-kitura.yaml
fi;
